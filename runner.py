...
class Runner:
    def __init__(self, cfg, model, lm_model, datasets, job_id):
        self.config = cfg
        ...
        self.lm_model = lm_model.to(self.device)
        self.lm_model.eval()
    ...
    def train_epoch(self, epoch):
        ...
        metric_logger.add_meter("normalizer", SmoothedValue(window_size=1, fmt="{value:.1f}"))
        metric_logger.add_meter("tau", SmoothedValue(window_size=1, fmt="{value:.3f}"))
        ...
        for i in metric_logger.log_every(range(self.iters_per_epoch), self.config.config.run.log_freq, header=header, logger=self.log_writter, start_step=epoch*self.iters_per_epoch):
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                max_steps = self.max_epoch * self.iters_per_epoch
                global_step = epoch * self.iters_per_epoch + i
                if self.config.config.run.tau_decay_steps < 1:
                    tau_decay_end_step = int(self.config.config.run.tau_decay_steps * max_steps)
                else:
                    tau_decay_end_step = int(self.config.config.run.tau_decay_steps)
                if global_step >= tau_decay_end_step:
                    tau_temp = self.config.config.run.tau_temp_end
                else:
                    decay_ratio = global_step / tau_decay_end_step
                    tau_temp = self.config.config.run.tau_temp_begin - decay_ratio * (self.config.config.run.tau_temp_begin - self.config.config.run.tau_temp_end)

                returned = self.lm_model(samples, self.model(None), tau_temp)
                loss = returned["loss"]
                weight_normalizer = returned['normalizer']
                tau_temp = returned['tau_temp']
            ...
            metric_logger.update(normalizer=weight_normalizer)
            metric_logger.update(tau=tau_temp)
    ...
    def valid_epoch(self, epoch, split, decode=False, save_json=False):
        ...
        for samples in metric_logger.log_every(dataloader, self.config.config.run.log_freq, header=header):
            ...
            if decode:
                ...
                text = self.lm_model.generate(samples, self.config.config.run, prompts=prompts, weight_tensor=self.model(None))
                res["text"] = text
                res["prompt"] = prompts
        ...
        res = {
            ...
            'normalizer': torch.tensor(0).float().cuda(),
        }
        for item in results:
            item_loss = item["loss"]
            ...
            res['normalizer'] += item['normalizer']
    ...
    @main_process
    def log_config(self):
        with open(os.path.join(self.output_dir, "log.json"), "a") as f:
