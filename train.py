...
from models import load_model, SimpleTensorModel
...
def main():
    ...
    model_config.norm_lambda = cfg.config.run.norm_lambda  # NOTE: dirty
    ...
    # build model
    lm_model = load_model(model_config)
    n_layers = lm_model.llama_model.config.num_hidden_layers
    n_heads = lm_model.llama_model.config.num_attention_heads
    embed_model = SimpleTensorModel(tensor_length=n_layers * n_heads + n_layers).to(lm_model.device)
    print("Embed model size:", embed_model.tensor.numel())
    ...
    # build runner
    runner = Runner(cfg, embed_model, lm_model, datasets, job_id)
