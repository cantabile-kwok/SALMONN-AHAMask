...
from .modeling_llama import LlamaForCausalLM  # NOTE: this includes attention head masks
from .modeling_whisper import WhisperModel
from .beats.BEATs import BEATsConfig, BEATs
from .utils import StoppingCriteriaSub, gumbel_sigmoid
...
class SALMONN(nn.Module):
    ...
    def __init__(
        ...
        self.speech_encoder = WhisperModel.from_pretrained(whisper_path, use_safetensors=False).encoder
        ...
        if autocast_dtype == "float16":
            ...
            raise NotImplementedError(f"{autocast_dtype} is not supported")
        print("Using autocast dtype ", autocast_dtype)
        
        # self.tau_decay_steps=tau_decay_steps
        # self.tau_temp_begin = tau_temp_begin
        # self.tau_temp_end = tau_temp_end
    ...
    def forward(self, samples, weight_tensor=None, tau_temp=1., verbose=False):
        bsz = len(samples['spectrogram'])
        weights_logit = weight_tensor.unsqueeze(0).repeat(bsz, 1)
        
        if self.use_gumbel:
            weights_tensor = gumbel_sigmoid(weights_logit, tau=tau_temp, hard=self.gumbel_hard)
        else:
        ...
        norm_loss = self.norm_lambda * normalizer
        ...
        loss = loss + norm_loss
        return {"loss": loss, "normalizer": normalizer.item(), "tau_temp": tau_temp}

    def generate(self, samples, generate_cfg, prompts=None, weight_tensor=None, prob_threshold=0.5):
        batch_size = samples["spectrogram"].shape[0]

        if weight_tensor is not None:
            weight_tensor = (weight_tensor.sigmoid() >= prob_threshold).float()
            weight_tensor = weight_tensor.unsqueeze(0).repeat(batch_size, 1).to(self.device)
        ...
        text = self.llama_tokenizer.batch_decode(outputs, skip_special_tokens=True, add_special_tokens=False)
    ...
    def from_config(cls, config):
        ...
        autocast_dtype = config.get("autocast_dtype", "float16")
        
        norm_lambda = config.get("norm_lambda", 1e-4)
        gumbel_hard = config.get("gumbel_hard", True)
