...
class SALMONNDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if sr!= 16000:
            # resample to 16000
            import torchaudio
            audio_tensor = torch.from_numpy(audio).T if audio.ndim > 1 else torch.from_numpy(audio).unsqueeze(0)
            ...
            if audio.ndim > 1:
                audio_resampled = audio_resampled.T.numpy()
            else:
                audio_resampled = audio_resampled.squeeze(0).numpy()
            audio_resampled = audio_resampled.astype(audio.dtype)
